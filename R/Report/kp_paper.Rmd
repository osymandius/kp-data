...
$$ Y_{a,t,x} \sim \text{Binom}(n, p_{a,t,x})$$ 
$$ p_{a,t,x} \sim \text{Beta}(?,?)$$ 
$$ logit(p_{a,t,x}) = \beta_0 + \beta_1 logit(\rho_{a,t,x})$$
$$\beta_{0:1} \sim N(0, 5)$$
### 3.3.3 ART coverage

We modelled key population ART coverage as a function of total population ART coverage, analogously to HIV prevalence. Missing denominators were imputed with n=38, the 25th centile of known denominators. Logit-transformed key population ART coverage is assumed to follow a betabinomial distribution against logit total population coverage.
...
$$ p_{a,t,x} \sim \text{Beta}(?,?)$$ 
$$ logit(p_{a,t,x}) = \beta_0 + \beta_1 logit(\alpha_{a,t,x})$$ 
$$\beta_{0:1} \sim N(0, 5)$$
...
imp_denomin <- quantile(filter(art_final, !is.na(denominator))$denominator, 0.25)
