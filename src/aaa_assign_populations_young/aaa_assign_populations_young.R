...
fsw_pop <- population %>%
  mutate(fsw_age = ifelse(age_group %in% c("Y015_019", "Y020_024", "Y025_029", "Y030_034", "Y035_039"), 1, 0)) %>%
  filter(fsw_age == 1) %>%
  group_by(area_id, area_name, year, sex) %>%
  summarise(population = sum(population)) %>%
  mutate(age_group = "Y015_039")
...
if(nrow(pse)) {
    ...
    pse <- pse %>%
      ...
      mutate(
        ...
        sample = NA,
        age_group = case_when(
          is.na(age_group) & kp %in% c("PWID") ~ "Y015_049",
          is.na(age_group) & kp %in% c("MSM", "TG", "TGW", "TGM") ~ "Y015_029",
          is.na(age_group) & kp == "FSW" ~ "Y015_039",
          TRUE ~ age_group
        ),
        notes = NA,
        link = NA) %>%
